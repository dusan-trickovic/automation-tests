...
abstract class BaseRepository {
  ...
  async findIssueByTitle(title: string): Promise<GitHubIssue | undefined> {
    const allOpenIssues = await this.fetchAllOpenIssues();
    const issue = allOpenIssues.find(
      (issue: GitHubIssue) => issue.title === title
    );
    return issue;
  }
}
...
export class ManifestRepository extends BaseRepository {
  constructor(
    owner: string,
    repo: string,
    public path: string = 'versions-manifest.json'
  ) {
    super(owner, repo);
  }

  async getVersionsManifestFromRepo(
    referenceVersion: string
  ): Promise<INecessaryDataFromManifest[]> {
    try {
      ...
      const latestFromManifest: INecessaryDataFromManifest[] =
        reversedJsonData.filter((item: INecessaryDataFromManifest) => {
...
export class GitHubIssue {
  ...
  async sendIssueInfoToSlack(toolName: string, expiringToolVersion: string) {
    ...
    try {
      ...
      core.info(successMessage);
        ...
        core.setFailed(
            'Error while sending the notification to Slack: ' + errorMessage
        );
    }
  }
